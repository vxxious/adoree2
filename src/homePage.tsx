    ...
    <section className="relative min-h-screen flex items-center justify-center px-3 xs:px-4 sm:px-6 pt-24 pb-12 sm:pt-24">
      <div className="max-w-7xl mx-auto w-full grid md:grid-cols-2 gap-6 sm:gap-8 md:gap-12 items-center">
        ...
        <div className="relative w-full h-64 xs:h-72 sm:h-80 md:h-96 lg:h-[600px] flex items-center justify-center order-1 md:order-2">
          <div className="cake-grid w-full h-full">
            <div className="cake-card cake-1 animate-float-delay-0">
              <img
                src="https://images.pexels.com/photos/1721934/pexels-photo-1721934.jpeg?auto=compress&cs=tinysrgb&w=800"
                alt="Decorated cake"
                className="rounded-lg xs:rounded-xl sm:rounded-2xl shadow-lg xs:shadow-xl sm:shadow-2xl object-cover w-full h-full"
              />
            </div>
            <div className="cake-card cake-2 animate-float-delay-1">
              <img
            ...
            <div className="cake-card cake-3 animate-float-delay-2">
              <img
  ...
  const MenuPage = () => (
    <section className="relative min-h-screen flex items-center justify-center px-3 xs:px-4 sm:px-6 pt-24 pb-12">
      <div className="max-w-4xl mx-auto w-full space-y-6 sm:space-y-8">
        ...
        <h2 className="text-2xl xs:text-3xl sm:text-4xl font-bold text-gray-800">Our Cake Menu</h2>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 xs:gap-5 sm:gap-6">
          {[
            { name: 'Classic Chocolate', price: '$45', desc: 'Rich, moist chocolate cake with fudgy frosting' },
            { name: 'Vanilla Dream', price: '$40', desc: 'Elegant vanilla cake with cream cheese frosting' },
            { name: 'Strawberry Bliss', price: '$50', desc: 'Fresh strawberries with light vanilla sponge' },
            { name: 'Red Velvet', price: '$55', desc: 'Traditional red velvet with cream cheese frosting' },
            { name: 'Lemon Zest', price: '$45', desc: 'Bright lemon cake with tangy lemon frosting' },
            { name: 'Carrot Cake', price: '$50', desc: 'Moist carrot cake with walnuts and cream cheese' },
          ].map((cake, idx) => (
            <div key={idx} className="bg-white rounded-xl shadow-lg p-4 xs:p-5 sm:p-6 hover:shadow-xl transition-shadow">
              <h3 className="text-base xs:text-lg sm:text-xl font-bold text-gray-800 mb-2">{cake.name}</h3>
              <p className="text-xs xs:text-sm text-gray-600 mb-3">{cake.desc}</p>
              <p className="text-xl xs:text-2xl font-bold text-pink-500">{cake.price}</p>
            </div>
          ))}
        </div>

        <div className="bg-pink-50 rounded-xl p-4 xs:p-5 sm:p-6 space-y-3">
          <h3 className="text-base xs:text-lg font-semibold text-gray-800">Custom Orders Available</h3>
          <p className="text-xs xs:text-sm text-gray-600">Don't see what you're looking for? We create custom cakes for any occasion. Contact us to discuss your unique cake design!</p>
          <button
            onClick={() => navigateTo('order')}
            className="w-full px-4 py-2.5 xs:py-3 bg-pink-500 text-white rounded-full font-semibold text-sm xs:text-base hover:bg-pink-600 transition-colors active:scale-95 touch-manipulation"
          >
            Place Custom Order
          </button>
        </div>
